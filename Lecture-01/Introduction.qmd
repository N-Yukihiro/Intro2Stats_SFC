...
インストールされていなければパッケージをインストールの上読み込み, パッケージに更新があればアップデートの上で読み込み, 特に何もなければ読み込みを行ってくれます.
...
g <- data.frame(
  Int  = 1:300000, 
  Int2 = seq(from = 0, by = 2.5, length.out = 300000), 
...
今開いているquartoのファイル上でdataというディレクトリを作成した場合(28行目)には, csvの書き出しの場所は以下のようにしてください.

```{r}
write.csv(g, here("Lecture-01", "data", "data.csv"), 
          quote = FALSE, row.names = FALSE)
```
